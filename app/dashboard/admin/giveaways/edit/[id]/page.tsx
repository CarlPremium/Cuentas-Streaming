    ...
    const fetchGiveaway = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Format the end_date for datetime-local input
      ...
      if (response.ok) {
        ...
        if (response.status === 401) {
          toast.error('Not authorized', {
            description: 'Please sign in to edit giveaways',
          })
        } else if (response.status === 403) {
          toast.error('Access denied', {
            description: 'You do not have permission to edit this giveaway',
          })
        } else {
      ...
      <main className="flex-1 overflow-auto">
        <div className="mx-auto max-w-3xl space-y-6 p-6 pb-36">
          <PageHeader
            icon="Pencil"
            title="Edit Giveaway"
            description="Update giveaway information"
          />
          <Card>
            <CardHeader>
        ...
        <form onSubmit={handleSubmit}>
          <Card>
            <CardHeader>
              <CardTitle>Giveaway Details</CardTitle>
              <CardDescription>
                Modify the information about your giveaway
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Title */}
